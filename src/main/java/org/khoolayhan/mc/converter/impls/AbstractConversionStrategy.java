...
/**
 * Abstract base class implementing the Template Method pattern for file conversion strategies.
...
public abstract class AbstractConversionStrategy implements ConversionStrategy {
	...
	/**
	 * Template method that orchestrates the conversion process.
	 * This method is final to prevent subclasses from overriding the conversion workflow.
	 * It performs the following steps:
	 * <ol>
	 *   <li>Validates that the input file exists</li>
	 *   <li>Logs the conversion attempt</li>
	 *   <li>Delegates to {@link #convertInternal(File, File)} for actual conversion</li>
	 *   <li>Catches and wraps Jackson exceptions in ConversionException</li>
	 * </ol>
	 *
	 ...
	 * @throws ConversionException if the input file does not exist, or if the conversion
	 *         process fails for any reason (wraps underlying exceptions)
	 */
    @Override
	...
	/**
	 * Performs the actual conversion logic specific to each strategy.
	 * This method is called by the template method {@link #convert(File, File)} after
	 * all validation and setup is complete. Subclasses must implement this method to
	 * provide their specific conversion behavior.
	 *
	 * @param input the validated source file to convert
	 * @param output the destination file for the conversion result
	 * @throws JacksonException if JSON/XML/CSV processing fails (will be caught and wrapped
