 ...
 * 1. High Coupling: Direct dependencies on Jackson classes inside the main logic.
 * 2. High Complexity: Nested if-else statements (High Cyclomatic Complexity).
 * 3. Low Cohesion: One class handles args parsing, file checking, and conversion logic.
 * 4. Violation of OCP: To add a new format, you must modify this file.
 */
public class LegacyMediaConverter {
	public static void main(String[] args) {
		...
		if (inputFileParam == null || outputFileParam == null) {
			System.err.println("Usage: java org.khoolayhan.mc.legacy.LegacyMediaConverter -i <input> -o <output>");
			return;
		}
		...
		try {
			...
			if ("json".equals(inExt)) {
				...
				CollectionType listType = jsonMapper.getTypeFactory().constructCollectionType(List.class, User.class);
				...
				if ("csv".equals(outExt)) {
					...
					// Write XML
					XmlMapper xmlMapper = XmlMapper.builder().enable(SerializationFeature.INDENT_OUTPUT).build();
			...
			// --- Csv Input ---
				...
				try (MappingIterator<User> it = csvMapper.readerFor(User.class).with(schema).readValues(inputFile)) {
			...
			// --- Xml Input ---
				...
				// Read XML
				XmlMapper xmlMapper = XmlMapper.builder().enable(SerializationFeature.INDENT_OUTPUT).build();
				CollectionType listType = xmlMapper.getTypeFactory().constructCollectionType(List.class, User.class);
